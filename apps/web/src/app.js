...
async function refreshDashboardData() {
  ...
  elements.metricSessionsMeta.textContent = `${countByStatus(state.appointments, 'scheduled')} ${t('status.scheduled').toLowerCase()} today`;
  elements.metricRolesValue.textContent = `${metadata.roles.length}`;
  elements.metricRolesMeta.textContent = 'Loaded from live bootstrap metadata';
  elements.metricApptValue.textContent = `${(metadata.appointmentTypes ?? []).length}`;
  elements.metricApptMeta.textContent = 'Appointment type model is in sync';
  elements.metricAuditValue.textContent = 'Synced';
  elements.metricAuditMeta.textContent = `Last event · ${formatTime(metadata.bootstrapEvent.occurredAt)}`;
